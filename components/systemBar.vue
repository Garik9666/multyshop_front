<template>
  <div>
    ...
    <v-system-bar height="64" color="#f39513" fixed app dark id="create" >
      <v-col cols="5">
      ...
      <v-speed-dial class="hidden-lg-and-up"  v-model="fab" :top="top" :bottom="bottom" :right="right" :left="left" :direction="direction" :open-on-hover="hover" :transition="transition" >
        <template v-slot:activator>
          <v-btn small  v-model="fab" color="#ea5a21" dark fab >
            <v-icon style="margin: 0;" v-if="fab">mdi-close</v-icon>
            <v-icon style="margin: 0;" v-else>mdi-account-circle</v-icon>
          </v-btn>
        </template>
        <v-badge color="error" content="6" >
          <v-btn to="/wishlist" fab color="#ea5a21" class="my-2 nav_button" small >
            <v-icon>mdi-heart-outline</v-icon>
          </v-btn>
        </v-badge>
        <v-badge color="error" content="6" >
          <v-btn to="/cart" fab color="#ea5a21" class="my-2 nav_button" small >
            <v-icon >mdi-cart-outline</v-icon>
          </v-btn>
        </v-badge>
        ...
        <v-menu v-model="menu" :close-on-content-click="false" :nudge-width="200" offset-x >
          <template v-slot:activator="{ on }">
            <v-btn fab color="#ea5a21" class="my-2 nav_button" small v-on="on" >
              <v-icon style="margin: 0;">mdi-account-outline</v-icon>
            </v-btn>
          </template>
        </v-menu>
        <v-btn v-for="(item, key) in languages" :key="key" to="/cart" fab color="#ea5a21" class="my-2 nav_button" small >
          <v-img :src="item.icon" max-width="50"></v-img>
        </v-btn>
      </v-speed-dial>
    </v-system-bar>
